...
def plot_landmarks_2d(save_dir, train, name, image, landmarks, landmarks_gt):
    ...
    """
    landmarks_gt_tv = np.vstack((landmarks_gt[0:2], landmarks_gt[2]))
    landmarks_gt_tc = landmarks_gt[0:2]
    landmarks_tv = np.vstack((landmarks[0:2], landmarks[2]))
...
def plot_landmarks_3d(save_dir, train, name, landmarks_mean, landmarks_gt, dim):
    ...
    fig = plt.figure()
    ax = fig.add_subplot(111)
    ax.plot(landmarks_mean[:, 0], landmarks_mean[:, 1], 'g.')
    ax.plot(landmarks_gt[:, 0], landmarks_gt[:, 1], 'r.')
    ax.set_title('{}'.format(name))
    ax.set_xlabel('x')
    ax.set_ylabel('y')
    ax.set_xlim([0, dim[0]])
...
def plot_landmarks_path(save_dir, train, name, landmarks_all_steps, landmarks_gt, dim):
    ...
    ax = fig.add_subplot(111)
    fig.set_tight_layout(True)
    ax.plot(landmarks_gt[:, 0], landmarks_gt[:, 1], 'rx')
    pt = []
    for j in range(num_landmarks):
        pt.append(ax.plot(landmarks_all_steps[0, :, j, 0], landmarks_all_steps[0, :, j, 1],
                            '.', c=c[j])[0])
